...
function calcularAgua (){
    infoPlanetas.forEach(element => suma += element.agua);
    return suma
}
...
function calcularAgua100 (){
    infoPlanetas.forEach(element => sumaPor100 += element.agua*100);
    return sumaPor100
}
...
function encontrarCeroH2O(){
    ceroH2O = infoPlanetas.find(item => item.agua == 0)
    return ceroH2O
}
